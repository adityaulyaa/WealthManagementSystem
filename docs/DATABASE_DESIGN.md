...
❌ **Historical Tracking**: No goal_progress history table
❌ **Transaction Recording**: No transaction ledger
❌ **Real-time Pricing**: Static asset prices
❌ **Portfolio Rebalancing**: No rebalancing history
❌ **Income/Expense Tracking**: Simplified tracking only
❌ **Social Features**: No user comparison data
...
❌ goal_progress        → Calculate at runtime
❌ insights             → Generate on-demand
❌ asset_price          → Static price di asset table
❌ allocation           → Replaced by portfolio_asset join table
```
...
**Foreign Key**: `user_id` → user.id (UNIQUE constraint - 1:1 relationship)
